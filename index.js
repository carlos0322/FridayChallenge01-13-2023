...
connection.query("INSERT INTO customers (customerNumber, customerName, contactLastName, contactFirstName, phone, addressLine1, addressLine2, city, state, postalCode, country,creditLimit) VALUES (510, 'Stephanie', 'Lopez', 'Carlos', '9876543210', '123 main Steet', '456 2nd Street', 'Charlotte', 'NC', '28273', 'USA', '50000')", function (err, result) {
    if (err)
        throw err;
...
connection.query("UPDATE customers SET customerName = 'Stephanie' WHERE customerNumber = 510", function (err, result) {
    if (err)
        throw err;
...
connection.query("DELETE FROM customers WHERE addressLine2 = '456 2nd Street' ", function (err, result) {
    if (err)
        throw err;
